...
class PagoCuentasXCobrarService {
  ...
  Future<List<FormaPago>> getFormasPago() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        developer.log(
          'Error al obtener formas de pago: ${response.statusCode}',
        );
        return [];
      }
    } catch (e) {
  ...
  bool validarDatosPago(PagosCuentasXCobrar pago) {
    ...
    if (pago.cpCoId <= 0) {
      developer.log(
        '❌ Error validación: cpCoId debe ser mayor a 0. Valor actual: ${pago.cpCoId}',
      );
      return false;
    }
    ...
    if (pago.pagoMonto <= 0) {
      developer.log(
        '❌ Error validación: pagoMonto debe ser mayor a 0. Valor actual: ${pago.pagoMonto}',
      );
      return false;
    }
    ...
    if (pago.foPaId <= 0) {
      developer.log(
        '❌ Error validación: foPaId debe ser mayor a 0. Valor actual: ${pago.foPaId}',
      );
      return false;
    }
    ...
    if (pago.pagoNumeroReferencia.trim().isEmpty) {
      developer.log(
        '❌ Error validación: pagoNumeroReferencia no puede estar vacío. Valor actual: "${pago.pagoNumeroReferencia}"',
      );
      return false;
    }
    developer.log(
      '✅ pagoNumeroReferencia válido: "${pago.pagoNumeroReferencia}"',
    );

    // Validar pagoObservaciones
    if (pago.pagoObservaciones.trim().isEmpty) {
      developer.log(
        '❌ Error validación: pagoObservaciones no puede estar vacío. Valor actual: "${pago.pagoObservaciones}"',
      );
      return false;
    }
    ...
    if (pago.usuaCreacion <= 0) {
      developer.log(
        'Error validación: usuaCreacion debe ser mayor a 0. Valor actual: ${pago.usuaCreacion}',
      );
      return false;
    }
  ...
  Future<bool> verificarCuentaPorCobrar(int cpCoId) async {
    try {
      ...
      developer.log(
        '🔍 Verificando cuenta $cpCoId - Status: ${response.statusCode}',
      );
  ...
  Future<List<PagosCuentasXCobrar>> listarPagosPorCuenta(int cpCoId) async {
    try {
      final url = Uri.parse(
        '$_apiServer/PagosCuentasPorCobrar/ListarPorCuentaPorCobrar/$cpCoId',
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded is Map<String, dynamic> && decoded.containsKey('data')) {
          ...
          if (data is List) {
            ...
            return data
                .map((item) => PagosCuentasXCobrar.fromJson(item))
                .toList();
          } else {
          ...
          developer.log(
            '✅ Pagos encontrados (lista directa): ${decoded.length}',
          );
          return decoded
              .map((item) => PagosCuentasXCobrar.fromJson(item))
              .toList();
