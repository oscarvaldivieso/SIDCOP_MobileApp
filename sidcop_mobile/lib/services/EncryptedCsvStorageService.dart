...
class EncryptedCsvStorageService {
  ...
  static Future<bool> _saveEncryptedCsvData(
    ...
    try {
      ...
      for (var item in data) {
        csvData.add(headers.map((header) => item[header]?.toString() ?? '').toList());
      }
      ...
      developer.log('$dataType guardados en CSV cifrado: ${data.length} registros');
      return true;
    } catch (e) {
  ...
  static Future<List<Map<String, dynamic>>> _loadEncryptedCsvData(
    ...
    try {
      ...
      developer.log('$dataType cargados desde CSV cifrado: ${items.length} registros');
      return items;
    } catch (e) {
  ...
  static Future<bool> saveProductsData(List<Map<String, dynamic>> products) async {
    return await _saveEncryptedCsvData(_productsFileName, products, 'Productos');
  }
  ...
  static Future<bool> saveHomeData(Map<String, dynamic> homeData) async {
    ...
    return await _saveEncryptedCsvData(_homeDataFileName, homeDataList, 'Datos del Home');
  }
  
  /// Carga datos del Home desde CSV cifrado
  static Future<Map<String, dynamic>?> loadHomeData() async {
    final homeDataList = await _loadEncryptedCsvData(_homeDataFileName, 'Datos del Home');
  ...
  static Future<bool> verifyFileIntegrity(String fileName) async {
    try {
      ...
      developer.log('Integridad del archivo $fileName verificada correctamente');
      return true;
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> getStorageInfo() async {
    try {
      ...
      info['total_size_mb'] = (info['total_size_bytes'] / (1024 * 1024)).toStringAsFixed(2);
