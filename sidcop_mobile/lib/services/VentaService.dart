...
class VentaService {
  ...
  try {
    ...
    if (!contentType.toLowerCase().contains('application/json')) {
      print(' [VentaService] Advertencia: Content-Type no es JSON: $contentType');
      if (contentType.toLowerCase().contains('text/html')) {
        print(' [VentaService] El servidor devolvió HTML, probablemente una página de error');
        return {
          'success': false,
          'error': true,
          ...
          'rawResponse': response.body.length > 1000 ? '${response.body.substring(0, 1000)}...' : response.body,
        };
      }
    }
    ...
    try {
      ...
      if (response.body.isNotEmpty) {
        final preview = response.body.substring(0, response.body.length > 10 ? 10 : response.body.length);
...
Future<Map<String, dynamic>?> listarVentasPorVendedor(int vendedorId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(' [VentaService] Número de ventas: ${responseData['data']?.length ?? 0}');
        
        return {
          'success': true,
          'error': false,
          'message': responseData['message'] ?? 'Ventas obtenidas correctamente',
          'data': responseData['data'] ?? [],
          'code': responseData['code'] ?? 200,
        };
      } else {
        final errorMsg = responseData['message'] ?? 'Error desconocido';
        print(' [VentaService] Error al obtener ventas (${response.statusCode}): $errorMsg');
        return {
          'success': false,
          'error': true,
          'message': errorMsg,
