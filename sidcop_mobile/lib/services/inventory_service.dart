...
class InventoryService {
  ...
  Future<List<Map<String, dynamic>>> getInventoryByVendor(int vendorId) async {
    ...
    if (hasConnection) {
      ...
      throw Exception(
        'Sin conexión a internet y no hay datos offline disponibles. Conecta a internet para sincronizar por primera vez.',
      );
    }
  }

  /// Obtiene datos de inventario desde servidor
  Future<List<Map<String, dynamic>>> _fetchInventoryFromServer(
    int vendorId,
  ) async {
    final url = Uri.parse(
      '$_apiServer/InventarioBodegas/InventarioAsignado?Vend_Id=$vendorId',
    );
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          final saved = await OfflineDatabaseService.saveInventoryData(
            inventoryData,
          );
          if (saved) {
            debugPrint(
              '✅ Inventario actualizado y guardado offline: ${inventoryData.length} productos',
            );
          } else {
  ...
  void _syncInventoryInBackground(int vendorId) {
    Future.microtask(() async {
      try {
        ...
        debugPrint(
          '❌ Error en sincronización de inventario en segundo plano: $e',
        );
      }
    });
  }
  ...
  Future<List<Map<String, dynamic>>> _getOfflineInventoryData() async {
    try {
      ...
      if (offlineData.isNotEmpty) {
        debugPrint(
          '✅ Inventario cargado desde offline: ${offlineData.length} productos',
        );
        return offlineData;
      } else {
        debugPrint('⚠️ No hay datos de inventario offline disponibles');
        throw Exception(
          'No hay datos de inventario disponibles offline. Conecta a internet para sincronizar.',
        );
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getJornadaDetallada(int vendorId) async {
    ...
    if (hasConnection) {
      ...
      final url = Uri.parse(
        '$_apiServer/InventarioBodegas/JornadaDetallada/$vendorId',
      );
      try {
        final response = await http.get(
          url,
        ...
        if (response.statusCode == 200) {
          ...
          throw Exception(
            'Failed to load jornada detallada: ${response.statusCode}',
          );
        }
      } catch (e) {
        ...
        if (hasOfflineData) {
          ...
          final offlineData =
              await OfflineDatabaseService.loadJornadaDetalladaData();
          return offlineData!;
        } else {
          throw Exception(
            'No se pudo conectar al servidor y no hay datos offline de jornada detallada. Conecta a internet para sincronizar por primera vez.',
          );
        }
      }
    } else {
      ...
      if (hasOfflineData) {
        ...
        throw Exception(
          'Sin conexión a internet y no hay datos offline de jornada detallada. Conecta a internet para sincronizar por primera vez.',
        );
      }
    }
  }
  ...
  Future<Map<String, dynamic>?> startJornadaOfflineFirst(
    int vendorId,
    int usuaCreacion,
  ) async {
    final hasConnection = await _hasInternetConnection();
    ...
    if (hasConnection) {
      try {
        ...
        return {
          'message':
              'Jornada iniciada offline - se sincronizará cuando haya conexión',
          'offline': true,
          'vendorId': vendorId,
        };
      }
  ...
  Future<Map<String, dynamic>?> startJornada(
    ...
    final url = Uri.parse(
      '$_apiServer/InventarioBodegas/IniciarJornada?Vend_Id=$vendorId&Usuario_Creacion=$usuaCreacion',
    );
    try {
      final response = await http.get(
        url,
  ...
  Future<Map<String, dynamic>> getJornadaActiva(int vendorId) async {
    ...
    final estadoLocal = await JornadaOfflineService.obtenerEstadoJornadaLocal(
      vendorId,
    );
    ...
    if (hasConnection) {
      final url = Uri.parse(
        '$_apiServer/InventarioBodegas/JornadaActiva?Vend_Id=$vendorId',
      );
      try {
        final response = await http.get(
          url,
        ...
        if (response.statusCode == 200) {
          ...
          throw Exception(
            responseData['message'] ??
                'Error desconocido al obtener la jornada activa',
          );
        } else {
          throw Exception(
            'Error al obtener la jornada activa: ${response.statusCode}',
          );
        }
      } catch (e) {
  ...
  Future<Map<String, dynamic>?> closeJornadaOfflineFirst(int vendorId) async {
    ...
    if (hasConnection) {
      try {
        ...
        return {
          'message':
              'Jornada cerrada offline - se sincronizará cuando haya conexión',
          'offline': true,
          'totalProductos': 0,
          'totalInicial': 0,
          'totalFinal': 0,
  ...
  Future<Map<String, dynamic>?> closeJornada(int vendorId) async {
    final url = Uri.parse(
      '$_apiServer/InventarioBodegas/CierreJornada?Vend_Id=$vendorId',
    );
    try {
      final response = await http.get(
        url,
  ...
  Future<bool> syncJornadaOperations() async {
    try {
      ...
      final success =
          await JornadaOfflineService.sincronizarOperacionesPendientes();

      if (success) {
        debugPrint('✅ Operaciones de jornada sincronizadas exitosamente');
      } else {
        debugPrint(
          '⚠️ Algunas operaciones de jornada no se pudieron sincronizar',
        );
      }
  ...
  Future<bool> hasOfflineInventoryData() async {
    try {
      ...
      debugPrint(
        '🔍 Verificando datos offline: ${inventoryData.length} productos encontrados',
      );
