...
class PedidosScreenOffline {
  ...
  /// Obtiene pedidos online que han sido guardados en caché
  static Future<List<Map<String, dynamic>>> obtenerPedidosSincronizadosCache() async {
    try {
      final raw = await leerJson('pedidos_sincronizados.json');
      if (raw == null) return [];
      return List<Map<String, dynamic>>.from(raw as List);
    } catch (e) {
      print('Error obteniendo pedidos sincronizados del caché: $e');
      return [];
    }
  }

  /// Guarda un pedido online en el caché de sincronizados
  static Future<void> guardarPedidoSincronizadoCache(Map<String, dynamic> pedidoData) async {
    try {
      print('💾 Guardando pedido online en caché de sincronizados...');
      
      // Leer pedidos sincronizados existentes
      final raw = await leerJson('pedidos_sincronizados.json');
      List<dynamic> sincronizados = raw != null ? List.from(raw as List) : [];
      
      print('💾 Pedidos sincronizados existentes: ${sincronizados.length}');
      
      // Verificar si el pedido ya existe (por server_id o local_signature)
      final serverId = pedidoData['server_id'];
      final localSignature = pedidoData['local_signature'];
      
      bool pedidoExiste = false;
      for (int i = 0; i < sincronizados.length; i++) {
        final pedidoExistente = sincronizados[i];
        if ((serverId != null && pedidoExistente['server_id'] == serverId) ||
            (localSignature != null && pedidoExistente['local_signature'] == localSignature)) {
          // Actualizar pedido existente
          sincronizados[i] = pedidoData;
          pedidoExiste = true;
          print('💾 Pedido actualizado en caché: $localSignature');
          break;
        }
      }
      ...
      if (!pedidoExiste) {
        sincronizados.add(pedidoData);
        print('💾 Nuevo pedido agregado al caché: $localSignature');
      }
      
      print('💾 Total después de agregar/actualizar: ${sincronizados.length}');
      
      // Guardar la lista actualizada
      await guardarJson('pedidos_sincronizados.json', sincronizados);
      
      print('✅ Pedido online guardado en caché de sincronizados exitosamente');
      
    } catch (e) {
      print('❌ Error guardando pedido sincronizado en caché: $e');
      rethrow;
    }
  }

  /// Limpia pedidos antiguos del caché (opcional, para mantener el caché limpio)
  static Future<void> limpiarCacheAntiguo({int diasMaximos = 30}) async {
    try {
      print('🧹 Limpiando caché de pedidos antiguos (más de $diasMaximos días)...');
      
      final fechaLimite = DateTime.now().subtract(Duration(days: diasMaximos));
      
      // Limpiar pedidos sincronizados antiguos
      final sincronizados = await obtenerPedidosSincronizadosCache();
      final sincronizadosFiltrados = sincronizados.where((pedido) {
        final fechaCreacion = DateTime.tryParse(pedido['created_at'] ?? '');
        return fechaCreacion == null || fechaCreacion.isAfter(fechaLimite);
      }).toList();
      
      if (sincronizadosFiltrados.length != sincronizados.length) {
        await guardarJson('pedidos_sincronizados.json', sincronizadosFiltrados);
        print('🧹 Eliminados ${sincronizados.length - sincronizadosFiltrados.length} pedidos sincronizados antiguos');
      }
